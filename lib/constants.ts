export const APP_NAME = "VoiceHarvest";

export const GRAIN_PRODUCTS = [
  {
    id: "almonds",
    name: "Almonds",
    hindi: "बादाम",
    emoji: "🌰",
    price: 650,
    unit: "kg",
    description: "Premium quality California-style almonds, rich in protein and healthy fats",
    shelfLife: "12 months",
    image: "/almonds.webp",
  },
  {
    id: "brown-channa",
    name: "Brown Chickpeas",
    hindi: "काला चना",
    emoji: "🫘",
    price: 85,
    unit: "kg",
    description: "High protein kala chana, perfect for curries and salads",
    shelfLife: "18 months",
    image: "/brown_channa.webp",
  },
  {
    id: "dal",
    name: "Dal",
    hindi: "दाल",
    emoji: "🟡",
    price: 95,
    unit: "kg",
    description: "Split lentils dal, staple in Indian cuisine",
    shelfLife: "12 months",
    image: "/dal.webp",
  },
  {
    id: "pearl-millet",
    name: "Pearl Millet",
    hindi: "बाजरा",
    emoji: "🌾",
    price: 45,
    unit: "kg",
    description: "Nutrient-rich bajra, great for rotis and porridge",
    shelfLife: "6 months",
    image: "/pearl_millet.webp",
  },
  {
    id: "rice",
    name: "Basmati Rice",
    hindi: "चावल",
    emoji: "🍚",
    price: 120,
    unit: "kg",
    description: "Long grain aromatic basmati rice from Punjab",
    shelfLife: "24 months",
    image: "/rice.webp",
  },
  {
    id: "turmeric",
    name: "Turmeric",
    hindi: "हल्दी",
    emoji: "🟠",
    price: 180,
    unit: "kg",
    description: "Organic turmeric with high curcumin content",
    shelfLife: "24 months",
    image: "/turmeric.jpg",
  },
  {
    id: "jowar",
    name: "Sorghum",
    hindi: "ज्वार",
    emoji: "🌾",
    price: 42,
    unit: "kg",
    description: "Gluten-free jowar, perfect for healthy rotis",
    shelfLife: "8 months",
    image: "/jowar.avif",
  },
  {
    id: "wheat",
    name: "Wheat",
    hindi: "गेहूं",
    emoji: "🌾",
    price: 35,
    unit: "kg",
    description: "Premium quality wheat grains from Madhya Pradesh",
    shelfLife: "12 months",
    image: "/wheat.webp",
  },
  {
    id: "urad-dal",
    name: "Urad Dal",
    hindi: "उड़द दाल",
    emoji: "⚫",
    price: 110,
    unit: "kg",
    description: "Black gram dal, essential for South Indian cuisine",
    shelfLife: "12 months",
    image: "/urad_dal.jpg",
  },
  {
    id: "tamarind",
    name: "Tamarind",
    hindi: "इमली",
    emoji: "🟤",
    price: 75,
    unit: "kg",
    description: "Fresh tamarind, perfect for chutneys and curries",
    shelfLife: "6 months",
    image: "/tamerine.webp",
  },
] as const;

export const COLORS = {
  primary: "#1B5E20",
  highlight: "#76FF03",
  accent: "#FFB300",
  success: "#4CAF50",
  error: "#D32F2F",
  warning: "#FFB300",
  info: "#0288D1",
  bgMain: "#FFFBF0",
  bgSurface: "#FFFFFF",
  bgSubtle: "#F5F5F5",
  textPrimary: "#424242",
  textSecondary: "#757575",
  textMuted: "#BDBDBD",
} as const;

export const SPACING = {
  xs: "4px",
  sm: "8px",
  md: "16px",
  lg: "24px",
  xl: "32px",
  "2xl": "48px",
  "3xl": "64px",
} as const;

export const BREAKPOINTS = {
  xs: 320,
  sm: 640,
  md: 768,
  lg: 1024,
  xl: 1280,
  "2xl": 1536,
} as const;

export const ANIMATION_DURATIONS = {
  fast: 0.2,
  normal: 0.3,
  slow: 0.6,
  slower: 1,
} as const;

export const API_ROUTES = {
  auth: {
    login: "/api/auth/login",
    register: "/api/auth/register",
    logout: "/api/auth/logout",
    verifyToken: "/api/auth/verify-token",
    forgotPassword: "/api/auth/forgot-password",
  },
  listings: {
    base: "/api/listings",
    search: "/api/listings/search",
    byId: (id: string) => `/api/listings/${id}`,
  },
  orders: {
    base: "/api/orders",
    byId: (id: string) => `/api/orders/${id}`,
  },
  voice: {
    processCommand: "/api/voice/process-command",
    transcription: "/api/voice/transcription",
    status: "/api/voice/status",
  },
  user: {
    profile: "/api/user/profile",
    preferences: "/api/user/preferences",
  },
  health: "/api/health",
} as const;

export const ROUTES = {
  HOME: "/",
  LOGIN: "/auth/login",
  REGISTER: "/auth/register",
  FORGOT_PASSWORD: "/auth/forgot-password",
  DASHBOARD: "/dashboard",
  VOICE: "/dashboard/voice",
  LISTINGS: "/dashboard/listings",
  LISTINGS_CREATE: "/dashboard/listings/create",
  ORDERS: "/dashboard/orders",
  home: "/",
  auth: {
    login: "/auth/login",
    register: "/auth/register",
    forgotPassword: "/auth/forgot-password",
    verifyEmail: "/auth/verify-email",
  },
  dashboard: {
    home: "/dashboard",
    listings: "/dashboard/listings",
    createListing: "/dashboard/listings/create",
    listingDetail: (id: string) => `/dashboard/listings/${id}`,
    orders: "/dashboard/orders",
    orderDetail: (id: string) => `/dashboard/orders/${id}`,
    voiceChat: "/dashboard/voice-chat",
    history: "/dashboard/history",
    settings: "/dashboard/settings",
  },
} as const;

export const VALIDATION = {
  password: {
    minLength: 8,
    maxLength: 128,
  },
  name: {
    minLength: 2,
    maxLength: 100,
  },
  phone: {
    minLength: 10,
    maxLength: 15,
  },
  description: {
    maxLength: 500,
  },
  quantity: {
    min: 1,
    max: 100000,
  },
  price: {
    min: 1,
    max: 100000,
  },
} as const;

export const ERROR_MESSAGES = {
  auth: {
    invalidCredentials: "Invalid email or password",
    emailInUse: "This email is already registered",
    weakPassword: "Password must be at least 8 characters",
    userNotFound: "No account found with this email",
    networkError: "Network error. Please try again",
    genericError: "Something went wrong. Please try again",
  },
  validation: {
    required: "This field is required",
    invalidEmail: "Please enter a valid email address",
    invalidPhone: "Please enter a valid phone number",
    passwordMismatch: "Passwords do not match",
    minLength: (field: string, length: number) =>
      `${field} must be at least ${length} characters`,
    maxLength: (field: string, length: number) =>
      `${field} must be less than ${length} characters`,
  },
  voice: {
    notSupported: "Voice recording is not supported in your browser",
    permissionDenied: "Microphone permission denied",
    processingFailed: "Failed to process voice command",
    lowConfidence: "Could not understand clearly. Please speak again.",
  },
} as const;

export const TESTIMONIALS = [
  {
    quote:
      "When they showed me this app, I was amazed. I just spoke in Hindi and it understood everything. If this comes to market, it will change how we sell our crops. Finally, technology that works for us!",
    author: "Ramesh Yadav",
    role: "Wheat Farmer, Madhya Pradesh",
    rating: 5,
    context: "Interview conducted in December 2024",
  },
  {
    quote:
      "I cannot read or write well, so apps are difficult for me. But this? I just talk like I'm talking to a person. My son explained it and I understood immediately. This is what farmers need.",
    author: "Lakshmi Devi",
    role: "Vegetable Farmer, Tamil Nadu",
    rating: 5,
    context: "Interview conducted in December 2024",
  },
  {
    quote:
      "The middlemen take 30-40% of our earnings. If I can sell directly to buyers through voice, it will be a blessing. I am excited to see this product launch. Please bring it soon!",
    author: "Gurpreet Singh",
    role: "Rice Farmer, Punjab",
    rating: 5,
    context: "Interview conducted in December 2024",
  },
] as const;

export const FEATURES = [
  {
    icon: "mic",
    title: "No Literacy Required",
    description: "Designed for illiterate & semi-literate farmers",
    highlight: "Works on basic 2G phones",
  },
  {
    icon: "globe",
    title: "Native Languages",
    description: "Currently supporting English, Hindi & Tamil",
    highlight: "More languages coming with your support",
  },
  {
    icon: "trending",
    title: "Better Prices",
    description: "Earn 30-40% more by cutting middlemen",
    highlight: "Direct buyer-to-farmer connection",
  },
  {
    icon: "zap",
    title: "Easy Updates",
    description: "Update inventory with voice: 'Sold 25kg'",
    highlight: "Your listing updates instantly",
  },
  {
    icon: "map",
    title: "Live Tracking",
    description: "Track where your crop is",
    highlight: "Pickup to buyer, real-time",
  },
  {
    icon: "shield",
    title: "Guaranteed Trust",
    description: "Secure payments & verified buyers",
    highlight: "Your harvest is protected",
  },
] as const;

export const PROBLEM_STATS = [
  {
    icon: "📚",
    stat: "43-50%",
    label: "Rural farmer illiteracy rate",
    description: "Digital platforms exclude illiterate farmers from modern markets",
  },
  {
    icon: "💰",
    stat: "₹3.5L Cr",
    label: "Lost to middlemen annually",
    description: "Farmers lose 35% profit because they can't access direct buyers",
  },
  {
    icon: "📵",
    stat: "8-15%",
    label: "eNAM platform adoption",
    description: "Even government platforms fail because of literacy requirements",
  },
  {
    icon: "🚜",
    stat: "86%",
    label: "Small & marginal farmers",
    description: "Most farmers own less than 2 hectares, lacking market access",
  },
] as const;

export const HOW_IT_WORKS = [
  {
    icon: "🎤",
    title: "Speak Your Harvest",
    description:
      "Just call our number. Say 'I have 50kg wheat at ₹60/kg' in your language.",
  },
  {
    icon: "🤖",
    title: "AI Understands",
    description:
      "Our AI converts your voice to a listing instantly. No typing. No reading.",
  },
  {
    icon: "🛒",
    title: "Buyers Purchase",
    description: "Buyers see your listing and buy directly from you.",
  },
  {
    icon: "💵",
    title: "You Get Paid",
    description: "Get 30-40% better prices. No middlemen. Direct transaction.",
  },
] as const;
